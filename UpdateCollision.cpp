...
void UpdateCollision::projectilesWithUnits(std::list <AttackType*>& playerProjectiles, std::list <AttackType*>& monsterAttacks, Player* player, std::list <Unit*>& monsters) {
	...
	for (it_projectile = monsterAttacks.begin(); it_projectile != monsterAttacks.end(); it_projectile++) {
		if ((*it_projectile)->collision(player) && (*it_projectile)->canBeHitted(player)) { // Player hitted  
			...
			// Activate passives
			(*it_projectile)->getAttackPassivesManager().activateHitEnemy(monsterAttacks, player->getPassivesManager()->getBuffsManager());
			player->getPassivesManager()->activatePassives(PassiveActivateOn::TakeDamage, playerProjectiles, NULL);
		}
	}
	...
	for (it_projectile = playerProjectiles.begin(); it_projectile != playerProjectiles.end(); it_projectile++) {
		for (it_monster = monsters.begin(); it_monster != monsters.end(); it_monster++) {
			if ((*it_projectile)->collision(*it_monster) && (*it_projectile)->canBeHitted(*it_monster)) { // Monster hitted
				(*it_projectile)->setEnemyHitted(*it_monster);
				(*it_monster)->takeDamage((*it_projectile)->getDamage());

				// Activate passives
				(*it_projectile)->getAttackPassivesManager().activateHitEnemy(playerProjectiles, (*it_monster)->getPassivesManager()->getBuffsManager());
				(*it_monster)->getPassivesManager()->activatePassives(PassiveActivateOn::TakeDamage, playerProjectiles, NULL);
			}
		}
	}
		...
		for (y; y <= ((*it_proj)->getPositionY() + (*it_proj)->getRadiusY()) / map->fieldRect.h; y++) {
			...
			for (x; x <= ((*it_proj)->getPositionX() + (*it_proj)->getRadius()) / map->fieldRect.w; x++) {
				...
				if (!map->getField(x, y)->ground()) {
					(*it_proj)->onWallHit();
					(*it_proj)->getAttackPassivesManager().activateHitWall(projectiles, NULL);
				}
				else {
